...
func fakePaginatedHandler(r *http.Request, rq db.QueryRunner, pgQuerier db.PaginatedQuerier[ethdb.NFT], query string, queryParams []interface{}) (PaginatedResponse[ethdb.NFT], error) {
...
func TestNFTsGetHandler_WithContractAndTokenID(t *testing.T) {
	...
	origHandler := PaginatedQueryHandlerFunc
	PaginatedQueryHandlerFunc = fakePaginatedHandler
	defer func() {
		PaginatedQueryHandlerFunc = origHandler
	}()
